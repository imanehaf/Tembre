...
def main(src, nbEx):
    ...
    for params in pararch:
        ...
        try:
            ...
            files = listdir(join('results', archdir))
            for fil in files:
                path = join('results', archdir, fil)
                data = zsimCycles(path)
                pos = 20*i-j
                dum = list(params)
                dum.extend([files.index(fil), data])
                df.loc[pos] = dum
                j-=1
        except:
            print '\n=\n=\n=\n=\n ERROR: something wrong with ' +archdir+'\n=\n=\n=\n=\n'
            cmd = 'mv arch.cfg  results/err_%s.cfg'%(archdir)
            sp.call(cmd, shell=True)
            continue
    ...
    df.to_pickle('results/%d_arch.pickle'%(nbEx))
    return df
